...
import org.lzw.sms01.utils.AliAccessKey;
import org.lzw.sms01.utils.RandomStringTLUtils;
...
public class SmsAction {
    ...
    @RequestMapping("/mt")
    public String login(){
        return "mtlogin";//返回模板页面的地址,不要加ftl
    }
    ...
    public String sendMsg(String phoneNum,String token) throws ClientException {
        ...
        if (cusFromToken == null){
            return "error";
        }
        String dynamicKey = "dynamicNo_"+phoneNum;
        Long incr = redisService.incr(dynamicKey);
        if (incr>3){
            redisService.del(phoneNum);
            return "获取已达上限3次";
        }
        //每次获得新的验证码,都有3次机会,此时要将之前的计数清掉
        redisService.del("error_" + phoneNum);
...
//        }
        ...
        redisService.setExp(phoneNum,numeric,300L);
